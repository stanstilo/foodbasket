...
const EditBilling = ({history, match, handleSubmit, pristine, submitting }) => {
  const reduxState = useSelector((state) => state.authReducer);
  const user = reduxState.currentUser.email;
  const dispatch = useDispatch();
    useEffect(()=>{
        dispatch(getSingleBillngAddress(match.params.id))
    },[])
  ...
  return (
    <form className="billing-form" onSubmit={handleSubmit(updateBilling)}>
...
function mapStateToProps(state) {
  ...
  )(reduxForm({ form: "billing", enableReinitialize: true ,validateBilling})(EditBilling));
